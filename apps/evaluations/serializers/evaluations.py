...
from apps.evaluations.models import Evaluation, ExperienceLevel
...
class EvaluationResponseSerializer(serializers.ModelSerializer):
    ...
    def get_others_count(self, obj: Evaluation) -> int:
        return obj.questions.filter(technology=None).count()

    def get_is_editable(self, obj: Evaluation) -> bool:
        return not Evaluation.objects.filter(id=obj.id, attempts__started_at__isnull=False).exists()
